...
results → RBAC denial → audit log): [`docs/media/`](media/).*
...
| MCP Server | 🔜 roadmap | The vendor-neutral connector contracts (`src/server/connectors/`) are built for an MCP swap-in, and a ready-to-run prompt for exposing ReviewOps as an MCP server (mock OAuth 2.1) is next in queue: [`PROMPT_MCP_SERVER.md`](PROMPT_MCP_SERVER.md) |
...
The full phased plan lives in [`ROADMAP.md`](ROADMAP.md). Next in queue: the
...
[`PROMPT_MCP_SERVER.md`](PROMPT_MCP_SERVER.md).
